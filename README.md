...
means that it is assumed the hogel column images are in a folder called Oppenheimer_hogels5, and named Hogel_Column_1.png, Hogel_Column_2.png, and so on.  

The output folder is currently hardcoded as OppenHeimer_Out.  

N_A, N_B hardcode the number of hogels horizontally and vertically, respectively.  Note that it is assumed that there are N_A many hogel column images, each of height N_B.  This makes sense - each hogel column image encodes, for a fixed x coordinate, the hogel images with all different possible y coordinates.
